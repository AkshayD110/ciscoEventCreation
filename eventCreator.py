...
class eventCreator:
    ...
    # accesstoken = "Bearer OGU5Y2Q4ODMtOWE4Yi00OWI1LWEzMGYtMDJjMmYyYjcwOGZiMTc2N2U2YTYtMDEy"
    # roomid = sys.argv[2]
    ...
    def msg_distribution(self):
        ...
        print(
            f"normal text will be : {normaltext_count}, markdown text will be : {markdowntext_count}, file transfer count will be :{filetransfer_count}")
    ...
    def normaltext_msg(self, text, count):
        for i in range(count):
            current_room = choice(self.roomid)
            message = self.api.messages.create(current_room, text=f"some generic text here - {text} {i}")
            time.sleep(1)

    def markdowntext_msg(self, text, count):
        for i in range(count):
            current_room = choice(self.roomid)
            message = self.api.messages.create(current_room, markdown=f"**some generic text here - {text} {i}**")
            time.sleep(1)

    def filetrasfer_msg(self, text, count):
        for i in range(count):
            current_room = choice(self.roomid)
            message = self.api.messages.create(current_room, text="some generic text here ",
                                               files=["http://hanassets.nd.gov/images/product/test.png"])
            time.sleep(2)
    ...
    def all_restricted_msgs(self):
        ...
        print(
            f"Here are the rest_wordsCount :{restrictedwords_count}, restricted_normal :{restricted_normal}, restricted_markdown :{restricted_markdown}, restricted_files :{restricted_files}")

        for i in range(restricted_normal):
            restricted_word = choice(listofphrase)
            current_room = choice(self.roomid)
            message = self.api.messages.create(current_room,
                                               text=f"some generic text here and restword : {restricted_word}")
            time.sleep(1)
        ...
        for i in range(restricted_markdown):
            ...
            current_room = choice(self.roomid)
            message = self.api.messages.create(current_room,
                                               markdown=f"**some generic text here and restword : {restricted_word}**")
            time.sleep(1)
